...
namespace PhotonBombFire
{
    public class EJBombFire : MonoBehaviourPun
    {
        ...
        void Update()
        {
            if (Input.GetKeyDown(KeyCode.Alpha3))
            {
                if (isBombDone)
                {
                    ...
                    photonView.RPC(nameof(MakeBombByRPC), RpcTarget.All);
                }
            }
        }

        [PunRPC]
        public void photonViewMakeBomb()
        {
            photonView.RPC(nameof(MakeBombByRPC), RpcTarget.All);
        }
        ...
        void StartMakeBomb()
        {
            StartCoroutine(MakeBombByRPC(null));
        }

        [PunRPC]
        public IEnumerator MakeBombByRPC(System.Action<int> complete)
        {
            isBombDone = false;
