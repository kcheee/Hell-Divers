...
namespace PhotonMachineGun
{
    public class EJMachineGun : MonoBehaviourPun
    {
        ...
        void Update()
        {
            if (Input.GetKeyDown(KeyCode.Alpha2))
            {
                if (isMachineDone)
                {
                    ...
                    photonView.RPC(nameof(MachineGunFireByRPC), RpcTarget.All);
                }
            }
        ...
        {
            StartCoroutine(MachineGunFireByRPC(null));
        }

        [PunRPC]
        public IEnumerator MachineGunFireByRPC(System.Action<int> complete)
        {
            RaycastHit machineGunHitInfo;
